...
using State = BlowFish::State;
using PIKey = State::PIKey;
using SBoxes = State::SBoxes;

inline constexpr PIKey Default_PIKey = {{0x243f6a88,
                                         0x85a308d3,
                                         0x13198a2e,
                                         0x03707344,
                                         0xa4093822,
...
inline constexpr SBoxes Default_SBoxes = {{
...
static inline constexpr uint32_t low_uint32_t(uint64_t value) noexcept { return static_cast<uint32_t>(value); }
...
static inline constexpr uint32_t feistel(State const& state, uint32_t value) noexcept {
    const auto h = state.sboxes[0][get_byte<3>(value)] + state.sboxes[1][get_byte<2>(value)];
    return (h ^ state.sboxes[2][get_byte<1>(value)]) + state.sboxes[3][get_byte<0>(value)];
}

static inline constexpr uint64_t Encrypt(State const& state, uint64_t block) noexcept {
...
static inline constexpr uint64_t Decrypt(State const& state, uint64_t block) noexcept {
    auto lo = low_uint32_t(block);
    auto hi = high_uint32_t(block);
    lo ^= state.pikey[17];
...
static constexpr void Encrypt(State const& state, char* data, size_t size) noexcept {
    if (size < 8) {
        return;
    }
    size = (size / 8) * 8;
...
static constexpr void Decrypt(State const& state, char* data, size_t size) noexcept {
    if (size < 8) {
        return;
    }
    size = (size / 8) * 8;
    for (auto i = data; i != data + size; i += 8) {
        uint64_t block = read_block(i);
        block = Decrypt(state, block);
        write_block(i, block);
    }
}

static constexpr State Create(char const* key, size_t size) noexcept {
    State state = {Default_PIKey, Default_SBoxes};
    if (size > 56) {
        size = 56;
    }
    for (size_t i = 0; i != 18; i++) {
        std::array<uint8_t, 4> bytes = {
            static_cast<uint8_t>((state.pikey[i] >> 0)),
            static_cast<uint8_t>((state.pikey[i] >> 8)),
            static_cast<uint8_t>((state.pikey[i] >> 16)),
            static_cast<uint8_t>((state.pikey[i] >> 24)),
        };
    ...
    for (size_t i = 0; i != 18; i += 2) {
        block = Encrypt(state, block);
        state.pikey[i] = low_uint32_t(block);
        state.pikey[i + 1] = high_uint32_t(block);
    }
    for (size_t i = 0; i != 4; ++i) {
        for (size_t j = 0; j != 256; j += 2) {
            block = Encrypt(state, block);
            state.sboxes[i][j] = low_uint32_t(block);
            state.sboxes[i][j + 1] = high_uint32_t(block);
        }
    }
    return state;
}

BlowFish::BlowFish(std::string_view key) noexcept : state(Create(key.data(), key.size())) {}

void BlowFish::encrypt(char* data, size_t size) const noexcept { Encrypt(state, data, size); }

void BlowFish::decrypt(char* data, size_t size) const noexcept { Decrypt(state, data, size); }
