...
using ProtoVer = PROTO_VER(0.9.22.14);

std::string_view ProtoVer::name() const { return "0.9.22.14"; }
...
void ProtoVer::read(Data_in& io, PKT_S2C_ToggleFoW& value) const { io.num<uint32_t>(value.fromID); }
void ProtoVer::write(Data_out& io, PKT_S2C_ToggleFoW const& value) const { io.num<uint32_t>(value.fromID); }
